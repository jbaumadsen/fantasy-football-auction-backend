...
export const getYahooAuthUrl = async (
...
export const getYahooCallback = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (error) {
      return res.redirect(
        `${FRONTEND_ORIGIN}/oauth-error?provider=yahoo&error=${encodeURIComponent(
          error
    ...
    if (!code || !state) {
      ...
      if ((req.query as any).connected === "yahoo") {
...
export const deleteYahooAuth = async (
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
) => {
