...
function wrapYahooError(e: any): never {
...
export class YahooApiService {
  ...
  getLeagueSettings(userId: string, leagueKey: string) {
    return this.get(userId, `/league/${leagueKey}/settings`);
  }

  /** League teams */
  getLeagueTeams(userId: string, leagueKey: string) {
  ...
  getTeamRoster(userId: string, teamKey: string) {
  ...
  getPlayers(
    userId: string,
    leagueKey: string,
    opts?: { search?: string; start?: number; count?: number }
  ) {
  ...
  async testPlayerDataEndpoints(userId: string, leagueKey: string, playerId?: string) {
