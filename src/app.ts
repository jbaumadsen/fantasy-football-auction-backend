...
app.delete("/auth/yahoo/disconnect", appAuthMiddleware(["user"]), async (req: AuthenticatedRequest, res: Response) => {
...
app.get("/auth/yahoo/callback", async (req: Request, res: Response) => {
  ...
  const parsed = verifyState(state as string);
  if (!parsed?.userId) {
...
app.get("/api/yahoo/players", appAuthMiddleware(["user"]), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId;
...
app.get("/api/yahoo/games", appAuthMiddleware(["user"]), async (req: AuthenticatedRequest, res: Response) => {
...
// --- League settings ---
app.get("/api/yahoo/league/settings", appAuthMiddleware(["user"]), async (req: AuthenticatedRequest, res: Response) => {
  try {
...
app.get("/api/yahoo/league/draftresults", appAuthMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { leagueKey } = req.query as unknown as { leagueKey: string };
    if (!leagueKey) return res.status(400).json({ error: "leagueKey required" });
    if (!req.userId) return res.status(401).json({ error: "Unauthorized" });
...
// --- League teams ---
app.get("/api/yahoo/league/teams", appAuthMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
app.get("/api/yahoo/team/roster", appAuthMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
